...
import UIKit
import Alamofire
class AddAddressDataResponce: UIViewController {
    
    static let shared = AddAddressDataResponce()
    public static func AddUserData(api:String,parameters:[String:Any],completion: @escaping(AddAddressDataStruct?)->Void)  {
        ...
        ApiService.shared.mobileApiRequest(api: api, method: .post, parameters: parameters) { (data, val, error) in
            ...
            do{
                let res = try JSONDecoder().decode(AddAddressDataStruct.self, from: data)
                completion(res)
            }catch{
                completion(nil)
